      ...
      <div className="container">
        <div className="features-inner">
          ...
          <div ref={gridRef} className="features-grid">
            {features.map((f, i) => (
              <div
                className={`feature-item reveal reveal-up${gridVisible ? ' visible' : ''} stagger-${(i % 6) + 1}`}
                key={f.title}
              >
                <div className="feature-icon" style={{ fontSize: 32, color: 'var(--primary)' }}>{f.icon}</div>
                <div>
                  <h4 className="text-sm font-bold mb-1" style={{ color: 'var(--text-primary)' }}>{f.title}</h4>
