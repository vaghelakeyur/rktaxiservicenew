...
function PkgDestImage({ pkg }) {
  ...
  return (
    <div className="pkg-dest-wrap">
      ...
      <span className="pkg-dest-fallback text-4xl" style={{ display: 'none' }}>{pkg.icon}</span>
    ...
    <section className="packages-section" id="packages">
      <div className="container">
        ...
        <div ref={gridRef} className="packages-grid">
          {packages.map((pkg, i) => (
            <div
              className={`pkg-card reveal reveal-up${gridVisible ? ' visible' : ''} stagger-${(i % 4) + 1}`}
              key={pkg.name}
            >
              <PkgDestImage pkg={pkg} />
              <div className="pkg-body">
                <h3 className="text-base font-bold mb-2" style={{ color: 'var(--text-primary)' }}>{pkg.name}</h3>
                <div className="pkg-meta">
                  <span className="flex items-center gap-1 text-sm" style={{ color: 'var(--text-secondary)' }}>
                    <MdCalendarMonth style={{ fontSize: 15 }} />{pkg.duration}
                  </span>
                  <span className="flex items-center gap-1 text-sm" style={{ color: 'var(--text-secondary)' }}>
                    <MdLocationOn style={{ fontSize: 15 }} />From {pkg.from}
                  </span>
                </div>
                <ul className="pkg-highlights">
                  {pkg.highlights.map(h => (
                    <li key={h} className="flex items-center gap-1 text-sm" style={{ color: 'var(--text-secondary)' }}>
                      <MdAutoAwesome style={{ fontSize: 13, color: '#f59e0b' }} />{h}
                    </li>
                  ))}
                </ul>
                <PkgCarImage pkg={pkg} />
        ...
        <div ref={ctaRef} className={`packages-cta reveal reveal-up${ctaVisible ? ' visible' : ''}`}>
          <p className="text-base font-medium" style={{ color: 'rgba(255,255,255,0.82)' }}>
            Don't see your destination? We customize packages for any location in India.
          </p>
